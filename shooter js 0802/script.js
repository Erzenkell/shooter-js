...
function creer_laser(){
    if(check_space == true && laser_tire == false){
        ...
        if (nb_tirs == 1){
            ...
            laser1.posY = vaisseau_posY + sprite_vaisseau.width/3;
            laser2.posX = vaisseau_posX + sprite_vaisseau.width;
            laser2.posY = vaisseau_posY + 2*sprite_vaisseau.width/3;
...
function Dessiner_explosion(){
    for(let i=0; i<Tableau_explosions.length; i++){
        setTimeout(() => {
            Tableau_explosions.splice(i,1);
        }, 200);
        ctx.drawImage(sprite_explosion, Tableau_explosions[i].x, Tableau_explosions[i].y);
    }
}
